...
class IndexPage extends React.Component {
	...
	toggleMode() {
		if (this.state.mode == 'theme-light') {
			this.setState({ mode: 'theme-dark' })
			...
			this.setState({ mode: 'theme-light' })
		...
		return (
			<Layout>
				...
				{ this.state.mode == 'theme-light' &&
					<button id='toggle-button' onClick={this.toggleMode} style={{ position: 'absolute', top: '25px', left: '25px' }}>dark</button>
				}
				{ this.state.mode == 'theme-dark' &&
					<button id='toggle-button' onClick={this.toggleMode} style={{ position: 'absolute', top: '25px', left: '25px' }}>light</button>
				}
				<div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
					<div style={{ position: 'relative' }}>
						<img id='room' src={this.state.mode == 'theme-light' ? RoomLight : RoomDark} alt='lilac room with brown floor' style={{ position: 'absolute', top: '0', left: '0', width: '900px', height: 'auto' }} />
						...
						<img id='glow' src={this.state.mode == 'theme-light' ? Sunlight : Moonlight} alt='rays of moonlight' style={{ position: 'absolute', top: '175px', left: '245px', width: '250px', height: 'auto', pointerEvents: 'none' }} />
					</div>
				</div>
			</Layout>
		)
